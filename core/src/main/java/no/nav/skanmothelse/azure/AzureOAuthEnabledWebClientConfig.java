...
public class AzureOAuthEnabledWebClientConfig {
	...
	@Bean
	List<ClientRegistration> clientRegistration(AzureProperties azureProperties, SkanmothelseProperties skanmothelseProperties) {
		return List.of(
				ClientRegistration.withRegistrationId(CLIENT_REGISTRATION_DOKARKIV)
						...
						.scope(skanmothelseProperties.getEndpoints().getDokarkiv().getScope())
						.build()
		);
	}
}
